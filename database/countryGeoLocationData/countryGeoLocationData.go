...
var countryGeoLocationDataSource = [][]string{
...
func BuildCountryGeoLocationData() {
	CountryGeoLocationData = make([]schemas.CountryGeoLocation, len(countryGeoLocationDataSource))
	for idx, item := range countryGeoLocationDataSource {
		countryCode := item[0]
		lat, _ := strconv.ParseFloat(item[1], 64)
		lon, _ := strconv.ParseFloat(item[2], 64)
		countryName := item[3]

		CountryGeoLocationData[idx] = schemas.CountryGeoLocation{
